...
const TM_xUserID = (()=>{try{const q=(s,r=document)=>r.querySelector(s),qa=(s,r=document)=>Array.from(r.querySelectorAll(s)),t=s=>typeof s=='string'?s.trim():%27%27,esc=s=%3E(s||%27%27).replace(/&/g,%27&%27).replace(/%3C/g,%27<%27).replace(/%3E/g,%27>%27).replace(/%22/g,%27"%27).replace(/%27/g,%27'');const%20url=q('meta[property=%22og:url%22]')?.content||location.href,u=new%20URL(url),parts=u.pathname.split('/').filter(Boolean),handleFromURL=parts[0]||'',USERNAME='@'+handleFromURL,spans=qa('div[data-testid=%22UserName%22]%20span,%20div[data-testid=%22User-Name%22]%20span'),displayName=spans.map(s=%3Et(s.textContent)).find(x=%3Ex&&!x.startsWith('@'))||'';const%20findIdentifier=()=%3E{const%20hn=(handleFromURL||'').toLowerCase(),escRe=s=%3Es.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const%20rx=[new%20RegExp('%22identifier%22:%22(\\d+)%22[\\s\\S]{0,15000}%22screen_name%22:%22'+escRe(hn)+'%22','i'),new%20RegExp('%22screen_name%22:%22'+escRe(hn)+'%22[\\s\\S]{0,15000}%22identifier%22:%22(\\d+)%22','i')];const%20scan=txt=%3E{if(!txt||txt.indexOf('%22identifier%22')===-1)return'';for(const%20r%20of%20rx){const%20m=txt.match(r);if(m)return%20m[1]}const%20m2=txt.match(/%22identifier%22:%22(\d{5,})%22/);return%20m2?m2[1]:''};for(const%20s%20of%20qa('script')){const%20x=s.textContent||'';const%20id=scan(x);if(id)return%20id}return%20scan(document.documentElement.innerHTML)||''};const%20identifier=findIdentifier();const%20findCreateISO=()=%3E{for(const%20s%20of%20qa('script')){const%20x=s.textContent||'';let%20m=x.match(/%22dateCreated%22:%22([^%22]+)%22/)||x.match(/%22created_at%22:%22([^%22]+)%22/)||x.match(/%22createdAt%22:%22([^%22]+)%22/);if(m){const%20d=new%20Date(m[1]);if(!isNaN(d))return%20d.toISOString()}}const%20joined=q('[data-testid=%22UserJoinDate%22]%20span')?.textContent||'';const%20jm=joined.match(/Joined\s+([A-Za-z]+)\s+(\d{4})/);if(jm){const%20d=new%20Date(jm[1]+'%201,%20'+jm[2]+'%2000:00:00Z');if(!isNaN(d))return%20d.toISOString()}return''};const%20createdISO=findCreateISO(),bannerImg=q('img[src*=%22pbs.twimg.com/profile_banners/%22]')?.src||'';const%20decodeSnowflakeMs=id=%3E{try{const%20EPOCH=1288834974657n;return%20Number((BigInt(id)%3E%3E22n)+EPOCH)}catch{return%20NaN}},parseBannerUpload=src=%3E{try{if(!src)return'';const%20clean=src.split('?')[0],m=clean.match(/\/profile_banners\/\d+\/(\d+)/);if(!m)return'';const%20token=m[1];let%20ms=NaN;if(/^\d{19,}$/.test(token)){ms=decodeSnowflakeMs(token)}else%20if(/^\d{13}$/.test(token)){ms=parseInt(token,10)}else%20if(/^\d{10}$/.test(token)){ms=parseInt(token,10)*1e3}if(!isNaN(ms)){const%20d=new%20Date(ms);if(!isNaN(d))return%20d.toISOString()}return'(unknown)'}catch{return'(unknown)'}};const%20bannUp=parseBannerUpload(bannerImg);let%20profileImg=q('img[src*=%22pbs.twimg.com/profile_images/%22]')?.src||q('meta[property=%22og:image%22]')?.content||q('img[alt*=%22profile%20picture%22]')?.src||'';const%20profileURL='https://x.com/'+handleFromURL,out=['Identifier:%20'+(identifier||'(unknown)'),'Profile%20URL:%20'+profileURL,'Username:%20'+(USERNAME||'(unknown)'),'Handle:%20'+(displayName||'(unknown)'),'Create%20Date:%20'+(createdISO||'(unknown)'),'Banner%20Image:%20'+(bannerImg||'(not%20found)'),'Banner%20Upload%20Date:%20'+(bannUp||'(unknown)')].join('\n');navigator.clipboard?.writeText(out).catch(()=%3E{});const%20id='__x_prof_overlay__';document.getElementById(id)?.remove();const%20box=document.createElement('div');box.id=id;box.style.cssText='position:fixed;z-index:2147483647;top:12px;right:12px;max-width:600px;background:#333;color:#fff;padding:12px;border-radius:12px;box-shadow:0%208px%2024px%20rgba(0,0,0,.35);font:14px/1.4%20system-ui,Segoe%20UI,Roboto,Helvetica,Arial';const%20imgHTML=profileImg?('%3Cimg%20src=%22'+esc(profileImg)+'%22%20alt=%22Avatar%22%20style=%22width:72px;height:72px;border-radius:50%;object-fit:cover;margin-right:10px;border:2px%20solid%20rgba(255,255,255,.2)%22%3E'):'%3Cdiv%20style=%22width:72px;height:72px;border-radius:50%;background:#333;margin-right:10px;display:flex;align-items:center;justify-content:center;color:#aaa%22%3EN/A%3C/div%3E';box.innerHTML='%3Cdiv%20style=%22display:flex;align-items:center%22%3E'+imgHTML+'%3Cdiv%20style=%22min-width:0%22%3E%3Cdiv%20style=%22font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis%22%3E'+esc(displayName||'(unknown)')+'%20%3Cspan%20style=%22opacity:.8%22%3E@'+esc(handleFromURL||'')+'%3C/span%3E%3C/div%3E%3C/div%3E%3C/div%3E%3Cdiv%20style=%22margin-top:8px;opacity:.9%22%3E%3Cdiv%3E%3Cstrong%3EIdentifier:%3C/strong%3E%20'+esc(identifier||'(unknown)')+'%3C/div%3E%3Cdiv%3E%3Cstrong%3EProfile%20URL:%3C/strong%3E%20'+esc(profileURL)+'%3C/div%3E%3Cdiv%3E%3Cstrong%3EUsername:%3C/strong%3E%20@'+esc(handleFromURL||'(unknown)')+'%3C/div%3E%3Cdiv%3E%3Cstrong%3EHandle:%3C/strong%3E%20'+esc(displayName||'(unknown)')+'%3C/div%3E%3Cdiv%3E%3Cstrong%3ECreate%20Date:%3C/strong%3E%20'+esc(createdISO||'(unknown)')+'%3C/div%3E%3Cdiv%3E%3Cstrong%3EBanner%20Image:%3C/strong%3E%20'+esc(bannerImg||'(not%20found)')+'%3C/div%3E%3Cdiv%3E%3Cstrong%3EBanner%20Upload%20Date:%3C/strong%3E%20'+esc(bannUp||'(unknown)')+'%3C/div%3E%3Cdiv%20style=%22display:flex;gap:8px;margin-top:10px%22%3E%3Cbutton%20id=%22__x_copy_btn__%22%20style=%22cursor:pointer;border:0;border-radius:8px;padding:6px%2010px;background:#0f62fe;color:#fff%22%3ECopy%3C/button%3E%3Cbutton%20id=%22__x_close_btn__%22%20style=%22cursor:pointer;border:0;border-radius:8px;padding:6px%2010px;background:#444;color:#fff%22%3EClose%3C/button%3E%3C/div%3E%3C/div%3E';document.body.appendChild(box);document.getElementById('__x_copy_btn__')?.addEventListener('click',()=%3Enavigator.clipboard?.writeText(out));document.getElementById('__x_close_btn__')?.addEventListener('click',()=%3Ebox.remove())}catch(e){alert('Bookmarklet%20error:%20'+(e&&e.message?e.message:e))}});
