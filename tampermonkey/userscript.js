...
const TM_xUserID = (()=>{try{const q=(s,r=document)=>r.querySelector(s),qa=(s,r=document)=>Array.from(r.querySelectorAll(s)),t=s=>typeof s=='string'?s.trim():'',esc=s=>(s||'').replace(/&/g,'&').replace(/</g,'<').replace(/>/g,'>').replace(/"/g,'"').replace(/'/g,''');const%20url=q('meta[property="og:url"]')?.content||location.href,u=new%20URL(url),parts=u.pathname.split('/').filter(Boolean),handleFromURL=parts[0]||'',USERNAME='@'+handleFromURL,spans=qa('div[data-testid="UserName"]%20span,%20div[data-testid="User-Name"]%20span'),displayName=spans.map(s=>t(s.textContent)).find(x=>x&&!x.startsWith('@'))||'';const%20findIdentifier=()=>{const%20hn=(handleFromURL||'').toLowerCase(),escRe=s=>s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const%20rx=[new%20RegExp('"identifier":"(\\d+)"[\\s\\S]{0,15000}"screen_name":"'+escRe(hn)+'"','i'),new%20RegExp('"screen_name":"'+escRe(hn)+'"[\\s\\S]{0,15000}"identifier":"(\\d+)"','i')];const%20scan=txt=>{if(!txt||txt.indexOf('"identifier"')===-1)return'';for(const%20r%20of%20rx){const%20m=txt.match(r);if(m)return%20m[1]}const%20m2=txt.match(/"identifier":"(\d{5,})"/);return%20m2?m2[1]:''};for(const%20s%20of%20qa('script')){const%20x=s.textContent||'';const%20id=scan(x);if(id)return%20id}return%20scan(document.documentElement.innerHTML)||''};const%20identifier=findIdentifier();const%20userIdURL=identifier?('https://x.com/i/user/'+identifier):'';const%20findCreateISO=()=>{for(const%20s%20of%20qa('script')){const%20x=s.textContent||'';let%20m=x.match(/"dateCreated":"([^"]+)"/)||x.match(/"created_at":"([^"]+)"/)||x.match(/"createdAt":"([^"]+)"/);if(m){const%20d=new%20Date(m[1]);if(!isNaN(d))return%20d.toISOString()}}const%20joined=q('[data-testid="UserJoinDate"]%20span')?.textContent||'';const%20jm=joined.match(/Joined\s+([A-Za-z]+)\s+(\d{4})/);if(jm){const%20d=new%20Date(jm[1]+'%201,%20'+jm[2]+'%2000:00:00Z');if(!isNaN(d))return%20d.toISOString()}return''};const%20createdISO=findCreateISO(),bannerImg=q('img[src*="pbs.twimg.com/profile_banners/"]')?.src||'';const%20decodeSnowflakeMs=id=>{try{const%20EPOCH=1288834974657n;return%20Number((BigInt(id)>>22n)+EPOCH)}catch{return%20NaN}},parseBannerUpload=src=>{try{if(!src)return'';const%20clean=src.split('?')[0],m=clean.match(/\/profile_banners\/\d+\/(\d+)/);if(!m)return'';const%20token=m[1];let%20ms=NaN;if(/^\d{19,}$/.test(token)){ms=decodeSnowflakeMs(token)}else%20if(/^\d{13}$/.test(token)){ms=parseInt(token,10)}else%20if(/^\d{10}$/.test(token)){ms=parseInt(token,10)*1e3}if(!isNaN(ms)){const%20d=new%20Date(ms);if(!isNaN(d))return%20d.toISOString()}return'(unknown)'}catch{return'(unknown)'}};const%20bannUp=parseBannerUpload(bannerImg);const%20findProfileImg=()=>{try{const%20hn=handleFromURL||'';let%20a=q('div[data-testid="primaryColumn"]%20a[href="/'+hn+'/photo"]')||q('a[href="/'+hn+'/photo"]');if(a){let%20im=a.querySelector('img[src*="pbs.twimg.com/profile_images/"]');if(im)return%20im.src;const%20els=[a,...qa('*',a)];for(const%20el%20of%20els){const%20bg=getComputedStyle(el).backgroundImage||'';const%20m=bg.match(/url\(["']?(https?:\/\/[^"')]+profile_images[^"')]+)["']?\)/i);if(m)return%20m[1]}}const%20og=q('meta[property="og:image"]')?.content||'';if(og&&/pbs\.twimg\.com\/profile_images\//i.test(og))return%20og;const%20escRe=s=>s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const%20hnre=escRe((hn||'').toLowerCase());const%20scan=txt=>{if(!txt||txt.indexOf('profile_image_url')===-1)return'';const%20raw=txt.replace(/\\\//g,'/');let%20m=raw.match(new%20RegExp('"screen_name":"'+hnre+'"[\\s\\S]{0,2000}"profile_image_url_https":"(https://pbs\\.twimg\\.com/profile_images/[^"]+)"','i'));if(m)return%20m[1];m=raw.match(new%20RegExp('"profile_image_url_https":"(https://pbs\\.twimg\\.com/profile_images/[^"]+)"[\\s\\S]{0,2000}"screen_name":"'+hnre+'"','i'));if(m)return%20m[1];m=raw.match(/"profile_image_url_https":"(https:\/\/pbs\.twimg\.com\/profile_images\/[^"]+)"/i);return%20m?m[1]:''};for(const%20s%20of%20qa('script')){const%20r=scan(s.textContent||'');if(r)return%20r}const%20r2=scan(document.documentElement.innerHTML);if(r2)return%20r2;const%20any=q('div[data-testid="primaryColumn"]%20img[src*="pbs.twimg.com/profile_images/"]');if(any)return%20any.src;return''}catch{return''}};let%20profileImg=findProfileImg();if(profileImg){profileImg=profileImg.replace('_normal','');if(profileImg.indexOf('name=')>-1)profileImg=profileImg.replace(/([?&])name=[^&]*/,'$1name=orig')}const%20profileURL='https://x.com/'+handleFromURL,out=['Identifier:%20'+(identifier||'(unknown)'),'User%20ID%20URL:%20'+(userIdURL||'(unknown)'),'Profile%20URL:%20'+profileURL,'Username:%20'+(USERNAME||'(unknown)'),'Handle:%20'+(displayName||'(unknown)'),'Create%20Date:%20'+(createdISO||'(unknown)'),'Banner%20Image:%20'+(bannerImg||'(not%20found)'),'Banner%20Upload%20Date:%20'+(bannUp||'(unknown)')].join('\\n');navigator.clipboard?.writeText(out).catch(()=>{});const%20id='__x_prof_overlay__';document.getElementById(id)?.remove();const%20box=document.createElement('div');box.id=id;box.style.cssText='position:fixed;z-index:2147483647;top:12px;right:12px;max-width:600px;background:#333;color:#fff;padding:12px;border-radius:12px;box-shadow:0%208px%2024px%20rgba(0,0,0,.35);font:14px/1.4%20system-ui,Segoe%20UI,Roboto,Helvetica,Arial';const%20imgHTML=profileImg?('<img%20src="'+esc(profileImg)+'"%20alt="Avatar"%20style="width:72px;height:72px;border-radius:50%;object-fit:cover;margin-right:10px;border:2px%20solid%20rgba(255,255,255,.2)">'):'<div%20style="width:72px;height:72px;border-radius:50%;background:#333;margin-right:10px;display:flex;align-items:center;justify-content:center;color:#aaa">N/A</div>';box.innerHTML='<div%20style="display:flex;align-items:center">'+imgHTML+'<div%20style="min-width:0"><div%20style="font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(displayName||'(unknown)')+'%20<span%20style="opacity:.8">@'+esc(handleFromURL||'')+'</span></div></div></div><div%20style="margin-top:8px;opacity:.9"><div><strong>Identifier:</strong>%20'+esc(identifier||'(unknown)')+'</div><div><strong>User%20ID%20URL:</strong>%20'+(userIdURL?('<a%20href="'+esc(userIdURL)+'"%20target="_blank"%20style="color:#9cf">'+esc(userIdURL)+'</a>'):'(unknown)')+'</div><div><strong>Profile%20URL:</strong>%20<a%20href="'+esc(profileURL)+'"%20target="_blank"%20style="color:#9cf">'+esc(profileURL)+'</a></div><div><strong>Username:</strong>%20@'+esc(handleFromURL||'(unknown)')+'</div><div><strong>Handle:</strong>%20'+esc(displayName||'(unknown)')+'</div><div><strong>Create%20Date:</strong>%20'+esc(createdISO||'(unknown)')+'</div><div><strong>Banner%20Image:</strong>%20'+esc(bannerImg||'(not%20found)')+'</div><div><strong>Banner%20Upload%20Date:</strong>%20'+esc(bannUp||'(unknown)')+'</div><div%20style="display:flex;gap:8px;margin-top:10px"><button%20id="__x_copy_btn__"%20style="cursor:pointer;border:0;border-radius:8px;padding:6px%2010px;background:#0f62fe;color:#fff">Copy</button><button%20id="__x_close_btn__"%20style="cursor:pointer;border:0;border-radius:8px;padding:6px%2010px;background:#444;color:#fff">Close</button></div></div>';document.body.appendChild(box);document.getElementById('__x_copy_btn__')?.addEventListener('click',()=>navigator.clipboard?.writeText(out));document.getElementById('__x_close_btn__')?.addEventListener('click',()=>box.remove())}catch(e){alert('Bookmarklet%20error:%20'+(e&&e.message?e.message:e))}});
